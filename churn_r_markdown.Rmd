...
## **Remover variables con valores unicos**
...
# **7. Modelamiento**
## **7.1 Selecionar las colmunas para entrenar el modelo**
...
## **7.2 Convertir datos categóricos en variables numéricas**
...
## **7.3 Guardar los datos para usarlos como conjunto de entrenamiento**
...
## **7.4  Probar distintos modelos de clasificación**
...
sample_size <- floor(0.05 * nrow(train_x))  # 1% of the number of rows
...
## **7.5 Configuración de los parámetros para la optimización de un modelo XGBoost**
...
## **7.6 Configurar el control de validación cruzada.**
...
## **7.7 Encontrar los mejores hiperparámetros**
...
xgb_tune <- train(
  ...
  method = "xgbTree",          # Generalized Boosted Regression Model (GBM)
  trControl = train_control,  # Cross-validation setup
  tuneGrid = tune_grid      # Hyperparameter grid for GBM
)

```

## **7.8 Imprimir los hiperparámetros**

```{r}
print(xgb_tune$bestTune)
```

## **7.9 Entrenar el modelo utilizando los hiperparámetros óptimos**
...
params <- list(
  ...
  max_depth = xgb_tune$bestTune$max_depth,                  # Max depth of trees
  eta = xgb_tune$bestTune$eta,                      # Learning rate
  nthread = 3,                    # Number of threads to use
  gamma = xgb_tune$bestTune$gamma,
  subsample = xgb_tune$bestTune$subsample,                # Fraction of samples used per tree
  colsample_bytree = xgb_tune$bestTune$colsample_bytree          # Fraction of features used per tree
)

# Train the model
model <- xgb.train(params, dtrain, nrounds = xgb_tune$bestTune$nrounds)
```
...
train_indices <- createDataPartition(train_y, p = 0.3, list = FALSE)

# Split the data into training and testing sets
train_x_split <- train_x[train_indices, ]  # Training predictors
train_y_split <- train_y[train_indices]    # Training target
test_x_split <- train_x[-train_indices, ]  # Testing predictors
test_y_split <- train_y[-train_indices]    # Testing target

```

## **7.10 Evaluar los resultados**
